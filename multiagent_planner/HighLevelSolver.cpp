// Major changes made in solve as the base code was wrong and did not produce a 
// path the current version works but fails in some edge cases.
...
std::vector<std::vector<Cell>> HighLevelSolver::solve(const Map &map) {
	...
	while (!isEmpty(tree)) {
		...
		std::vector<TreeNode> newTree;
		for (int i = 0; i < tree.size(); i++)
		{
			if ( tree[i].getCost()==P.getCost())
			{
				continue;
